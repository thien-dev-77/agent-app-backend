...
- Nếu có nhiều ảnh đầu vào, "Prompt video tổng hợp" bắt buộc chứa đủ tất cả tag theo thứ tự: <FIRST_FRAME>${imageCount > 1 ? `, ${Array.from({ length: imageCount - 1 }, (_, index) => `<IMAGE_REF_${index}>`).join(', ')}` : ''}. Dùng các tag này trong cùng một prompt liền mạch, ví dụ: "Starting from <FIRST_FRAME>, create a polished TVC commercial. Use <IMAGE_REF_0>, <IMAGE_REF_1> as visual continuity references for wardrobe, product details, props, camera motion, scene continuity, brand mood, and CTA..."
- Không được bỏ sót tag ảnh tham chiếu nào trong "Prompt video tổng hợp".
- Tối ưu riêng cho Google Omni Image-to-Video: trong "Prompt video tổng hợp", không mô tả lại ngoại hình chi tiết của người từ ảnh đầu vào như đẹp/xinh, màu tóc, màu váy, dáng mặt, tuổi, màu da, trang phục. Không dùng tên người nổi tiếng, tên người thật, hoặc các cụm yêu cầu nhận diện/copy danh tính cá nhân. Chỉ nhắc dùng ảnh làm visual continuity reference và tập trung mô tả chuyển động.
- "Prompt video tổng hợp" phải ưu tiên chuyển động tự nhiên dựa trên storyboard: micro-expressions, subtle eye blinking, gentle breathing movement, soft smile when appropriate, natural lip movement if speaking, small head/shoulder shifts, hand/finger motion, slight hair sway, realistic light reflections in eyes, dust particles or small environmental motion when phù hợp, và camera movement chậm/gọn.
- Tránh thêm chi tiết thừa có thể làm Omni vẽ lại khuôn mặt/người. Càng ít mô tả ngoại hình càng tự nhiên; càng rõ chuyển động nhỏ càng giống video thật.
